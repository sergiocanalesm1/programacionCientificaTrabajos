...
def doce():
    for Hz in range(1,11):#para graficar todas las frecuencias de 1 a 10
        x = np.linspace(0,10,1000)
        y = np.sin(x*Hz/(2*np.pi))#convertir a la frecuencia deseada (1-10Hz)
        plt.figure(Hz)#me crea multiples graficas
        plt.plot(x,y)
        plt.title("{}Hz sine graph".format(Hz))
        plt.ylabel('Sine')
        plt.xlabel('Time')
    plt.show()
...
def trece():
    ...
    for p in partitions:
        plt.figure(p)
        plt.hist(nums,bins=p)
    plt.show()


def catorce():
    nums = np.random.normal(0,1, 10000)
    partitions = [10, 20, 30, 40, 50]
    for p in partitions:
        plt.figure(p)
        plt.hist(nums,bins=p)
